...
| `<версия>` | `:0.0.2` | этот релиз репозитория, PHP по умолчанию |
| `<suite>-php<X.Y>` | `:trixie-php8.3` | свежий релиз на ветке PHP (двигается вперёд) |
| `<версия>-php<X.Y>` | `:0.0.2-php8.3` | релиз репозитория на ветке PHP |
...
Самодостаточный, ужесточённый по безопасности пример (маленький `Dockerfile`,
запекающий приложение и его конфиг в образ) лежит в [`examples/`](examples/) —
