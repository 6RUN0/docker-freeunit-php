...
| `<версия>` | `:0.0.9` | этот релиз репозитория, PHP по умолчанию |
| `<suite>-php<X.Y>` | `:trixie-php8.3` | свежий релиз на ветке PHP (двигается вперёд) |
| `<версия>-php<X.Y>` | `:0.0.9-php8.3` | релиз репозитория на ветке PHP |
...
При первом запуске (когда `/var/lib/freeunit` пуст) entrypoint поднимает демон на
...
|------------|--------------|------------|
| `APPLICATION_USER`  | `freeunit` | Пользователь приложения. Если уже существует, его UID сохраняется. |
| `APPLICATION_UID`   | `1000` | UID, используемый только при создании пользователя. |
| `APPLICATION_GROUP` | `freeunit` | Группа приложения. Если уже существует, её GID сохраняется. |
